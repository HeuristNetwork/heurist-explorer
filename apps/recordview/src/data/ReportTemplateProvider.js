 ...
 * @brief Loads Smarty report templates for the `smarty` engine's template picker.
 *
 ...
 * @since       8.0
 */
/** Provides configured Smarty report templates for the configuration dialog. */
export class ReportTemplateProvider {
  ...
  async list({ signal } = {}) {
    ...
    if (!response.ok)
      throw new Error(`Report template list request failed (${response.status})`);
...
function normalizeTemplates(value) {
  ...
  return source
    .map((item) => {
      ...
      const value =
        item?.value ?? item?.name ?? item?.filename ?? item?.file ?? item?.id ?? item?.key;
      return value == null || value === ""
        ? null
        : {
            value: String(value),
            label: String(item.label ?? item.title ?? item.name ?? item.filename ?? value),
          };
    })
    .filter(Boolean);
}
